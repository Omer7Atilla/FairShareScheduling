...
//Where we initialise the scheduler flag
int fss_flag = 0;
...
repeat_schedule:
	...
	//We can implement fair share scheduling by only changing how jiffies are distributed between processes in here
	...
	if (unlikely(!c)) {
		...
		if(fss_flag)
		{
			//Counters are shared based on how many processes that uid has
			//We don't have to count how many active uids in total
			//since every process gets 20 ticks by default the math still works
			...
			for_each_task(p) {
				...
				//hoca said not to change real-time processes scheduling logic
				//So the OS itself doesn't breaks
				if(p->policy != SCHED_OTHER | p->uid == 0)
				{
					p->counter = (p->counter >> 1) + NICE_TO_TICKS(p->nice);
				}
				else if(p->user)
				{
					int procs_in_uid = atomic_read(&p->user->processes);

					if(procs_in_uid > 0)
					{
						int fair_slice = NICE_TO_TICKS(p->nice) / procs_in_uid;

						if(fair_slice == 0) fair_slice = 1;
						
						p->counter = (p->counter >> 1) + fair_slice;
					}
				}
			}
